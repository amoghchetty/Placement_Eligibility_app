...
class Placement_table:
    ...
    def generate_placements(self):
        for _, row in self.students_df.iterrows():
            placement_status = random.choice(["Placed", "Eligible", "Not Eligible"])
            placement_info = {
                "placement_id": self.fake.unique.random_int(min=1000, max=9999),
                "student_id": row['student_id'],
                "mock_test_score": random.randint(50, 100),
                "project_experience": random.randint(0, 5),
...
#     st.title(" Search & Filter Students")
# 
#     search_name = st.text_input("Search by Student Name")
#     search_city = st.text_input("Filter by City")
#     enroll_year = st.selectbox("Filter by Enrollment Year", options=["All"] + sorted(students_df['enrollment_year'].unique().tolist()))
#     placement_status = st.selectbox("Filter by Placement Status", options=["All"] + sorted(placements_df['placement_status'].unique().tolist()))
#     company = st.text_input("Filter by Company Name")
# 
# 
...
#     if search_city:
#         merged_df = merged_df[merged_df['city'].str.contains(search_city, case=False)]
#     if enroll_year != "All":
#         merged_df = merged_df[merged_df['enrollment_year'] == int(enroll_year)]
#     if placement_status != "All":
#         merged_df = merged_df[merged_df['placement_status'] == placement_status]
#     if company:
#         merged_df = merged_df[merged_df['company_name'].fillna("").str.contains(company, case=False)]
# 
